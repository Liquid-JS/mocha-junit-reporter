...
export = function (stats: mocha.Stats | undefined, options?: any) {
    const data = {
        testsuites: [
            {
                _attr: {
        ...
        ] as [{ _attr: xml.XmlAttrs }, ...{ testsuite: Array<any> }[]]
    }

    if (options && options.properties) {
        const properties = {
            properties: [] as Array<xml.XmlObject>
        }
        ...
        const [_, ...test] = data.testsuites
        test.forEach(t => t.testsuite.push(properties))
    }

    if (stats?.pending) {
        (data.testsuites[0]._attr.tests as number) += stats.pending
