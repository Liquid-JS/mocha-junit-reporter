...
[![Build Status][github-badge]][github-build]
...
```shell
$ npm install @liquid-js/mocha-junit-reporter --save-dev
```

or as a global module

```shell
$ npm install -g @liquid-js/mocha-junit-reporter
```

## Usage

Run mocha with `@liquid-js/mocha-junit-reporter`:

```shell
$ mocha test --reporter @liquid-js/mocha-junit-reporter
```
...
```shell
$ MOCHA_FILE=./path_to_your/file.xml mocha test --reporter @liquid-js/mocha-junit-reporter
```

or

```shell
$ mocha test --reporter @liquid-js/mocha-junit-reporter --reporter-options mochaFile=./path_to_your/file.xml
```
...
```javascript
var mocha = new Mocha({
    reporter: '@liquid-js/mocha-junit-reporter',
    reporterOptions: {
...
```shell
PROPERTIES=BUILD_ID:4291 mocha test --reporter @liquid-js/mocha-junit-reporter
```
...
Results XML filename can contain `[hash]`, e.g. `./path_to_your/test-results.[hash].xml`. `[hash]` is replaced by MD5 hash of test results XML. This enables support of parallel execution of multiple `@liquid-js/mocha-junit-reporter`'s writing test results in separate files.
...
[github-build]: https://github.com/Liquid-JS/mocha-junit-reporter/actions/workflows/test.yml

[npm-badge]: https://img.shields.io/npm/v/@liquid-js/mocha-junit-reporter.svg?maxAge=2592000

[npm-listing]: https://www.npmjs.com/package/@liquid-js/mocha-junit-reporter
